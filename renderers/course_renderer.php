 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_celoe_core_course_renderer extends core_course_renderer
{
    ...
    public function frontpage_available_courses()
    {
        ...
        if (count($rcourseids) > 0) {
            ...
            foreach ($rcourseids as $courseid) {
                ...
                $no = get_config('theme_celoe', 'patternselect');
    ...
    public function promoted_courses()
    {
        ...
        $pcourseenable = theme_celoe_get_setting('pcourseenable');
        if (!$pcourseenable) {
            return false;
        }
        ...
        $featuredids = theme_celoe_get_setting('promotedcourses');
        $rcourseids = (!empty($featuredids)) ? explode(",", $featuredids) : array();
        if (empty($rcourseids)) {
            return false;
        }

        $hcourseids = theme_celoe_hidden_courses_ids();
        ...
        $promotedtitle = theme_celoe_get_setting('promotedtitle', 'format_html');
        $promotedtitle = theme_celoe_lang($promotedtitle);
        ...
        if (!empty($fcourseids)) {
            foreach ($fcourseids as $courseids) {
                ...
                foreach ($courseids as $courseid) {
                    ...
                    $summary = theme_celoe_strip_html_tags($course->summary);
                    $summary = theme_celoe_course_trim_char($summary, 75);
